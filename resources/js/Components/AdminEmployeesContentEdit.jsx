...
const AdminEmployeesContentEdit = ({ employee }) => {
    const {data, setData, patch, processing, errors } = useForm({
        id: employee.id,
        name: employee.name,
        email: employee.email,
        birth_date: new Date(employee.birth_date).toISOString().split('T')[0],
        phone_number: employee.phone_number,
        role: employee.role,
        role_status: employee.role_status,
    })

    const handleSubmit = (event) => {
        event.preventDefault();
        patch(route('admin.employees.update', data.id))
    };
    ...
    return (
        <>
            ...
            <h3 className="mt-5 font-bold text-4xl text-black">Edit Data Pegawai</h3>
            <form onSubmit={handleSubmit}>
                ...
                <button type="submit" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Perbarui Data</button>
            </form>
        </>
    )
}

export default AdminEmployeesContentEdit;
